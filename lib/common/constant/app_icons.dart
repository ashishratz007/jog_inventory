import 'package:jog_inventory/common/exports/common.dart';

import '../exports/main_export.dart';

class _AppIcons {
  static final _path = "assets/icons/";

  String boxes = "${_path}boxes.png";
  String scan = "${_path}scan.png";
  String boxes_white = "${_path}boxes_white.png";
  String future = "${_path}future.png";
  String mat_white = "${_path}mat_white.png";
  String no_code = "${_path}no_code.png";
  String fabric = "${_path}fabric.png";
  String edit = "${_path}edit.png";
  String double_arrow = "${_path}double_arrow.png";

  /// sidebar icons
  String forecast_sel = "${_path}forecast_sel.png";
  String forecast_un = "${_path}forecast_un.png";
  String material_req_sel = "${_path}material_req_sel.png";
  String material_req_un = "${_path}material_req_un.png";
  String no_code_req_un = "${_path}no_code_req_un.png";
  String no_code_req_sel = "${_path}no_code_req_sel.png";
  String stock_in_list_sel = "${_path}stock_in_list_sel.png";
  String stock_in_list_un = "${_path}stock_in_list_un.png";
  String paper = "${_path}paper.png";
  String paperColor = "${_path}paper_color.png";
  String ink = "${_path}ink.png";
  String inkColor = "${_path}ink_color.png";
  String forecast = "${_path}forecast.png";
  String forecast_color = "${_path}forecast_color.png";
  String material = "${_path}material.png";
  String material_color = "${_path}material_color.png";
  String no_code_color = "${_path}no_code_color.png";
  String no_code_unsel = "${_path}no_code_unsel.png";
  String stock_in = "${_path}stock_in.png";
  String stock_in_color = "${_path}stock_in_color.png";
}

var AppIcons = _AppIcons();

class Icomoon {
  Icomoon._();

  static const _kFontFam = 'Icomoon';
  static const String? _kFontPkg = null;
  //
  // static const IconData location =
  //     IconData(0xe900, fontFamily: _kFontFam, fontPackage: _kFontPkg);
// Add more icon mappings here...
}
